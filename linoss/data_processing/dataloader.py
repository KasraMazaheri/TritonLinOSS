...
3. `CoeffDataloader`: Used by NCDEs.

4. `PathDataloader`: Used by NRDEs and Log-NCDEs
...
class CoeffDataloader(BaseDataloader):
    """
    Batches and yields data for `NCDEs`, requiring coefficients for the data.

    Input data should be a tuple of length 3:
        - The first element: jnp.ndarray of shape (n_samples, n_timesteps)
        for sampling times.
        - The second element: a tuple of length n_coeffs, where each element
        is a jnp.ndarray of shape (n_samples, n_timesteps-1, n_features) for
        interpolation coefficients.
        - The third element: jnp.ndarray of shape (n_samples, n_features) for
        the initial value.
    """
    ...
    def _loop(self, batch_size, *, key):
        while True:
            ...
            if len(self.data) <= batch_size:
                ...
                idxs = jr.choice(
                    batch_key, self.size, shape=(batch_size,), replace=False
                )
                yield (
                    self.func(self.data[0][idxs]),
                    tuple(self.func(data[idxs]) for data in self.data[1]),
                    self.func(self.data[2][idxs]),
                ), self.func(self.labels[idxs])
...
class PathDataloader(BaseDataloader):
    """
    Batches and yields data for `NRDEs` and `Log-NCDEs`, requiring log-signatures
    for the data.

    Input data should be a tuple of length 3:
        - The first element: jnp.ndarray of shape (n_samples, n_timesteps)
        for sampling times.
        - The second element: jnp.ndarray of shape
        (n_samples, n_intervals, n_logsig_features) for log-signature data.
        - The third element: jnp.ndarray of shape (n_samples, n_features)
