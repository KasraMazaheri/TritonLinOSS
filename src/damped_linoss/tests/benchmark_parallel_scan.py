...
def generate_torch_data(B, L, P, VAR=1e-3, requires_grad=False):
	M = torch.randn(B, P * 4, dtype=torch.bfloat16, device='cuda', requires_grad=requires_grad) * VAR
	F = torch.randn(B, L, 2 * P, 2, dtype=torch.bfloat16, device='cuda', requires_grad=requires_grad) * VAR
	if requires_grad:
		M.retain_grad()
		F.retain_grad()
	return M, F
...
def generate_jax_data(B, L, P, VAR=1e-3, key=None):
	...
	M = jax.random.normal(key_M, (B, 4 * P,), dtype=jnp.bfloat16) * VAR
...
def benchmark_config(B, L, P, TILE_L, n_warmup, n_repeat):
	...
	def jax_forward(jM, jF):
		return jax.lax.associative_scan(binary_operator, (jM * jnp.ones((L, 4 * P)), jF))
