...
if __name__ == "__main__":
    # benchmark 5 runs each (ignoring compilation time)
    import time
    import jax
    ...
    x_jax = jax.device_put(x.cpu().numpy())
    result = associative_scan(add, x, reverse=False, axis=0)
    result_jax = jax.lax.associative_scan(add, x_jax, axis=0)

    print("Input:\n", x.squeeze().numpy())
    print("Scan result:\n", result.numpy().flatten())
    print("JAX Scan result:\n", result_jax.flatten())

    # compile versions
    def torch_scan(X):
        return associative_scan(add, X, reverse=False, axis=0)

    def jax_scan(X):
        return jax.lax.associative_scan(add, X, axis=0)

    compiled_torch = torch.compile(torch_scan)
    result_compiled = compiled_torch(x)
    print("Compiled Scan result:\n", result_compiled.numpy().flatten())

    compiled_jax = jax.jit(jax_scan)
    result_jax_compiled = compiled_jax(x_jax)
    print("Compiled JAX Scan result:\n", result_jax_compiled.flatten())
    ...
    for _ in range(n_runs):
        ...
        start = time.time()
        compiled_jax(x_jax).block_until_ready()
        jax_times.append(time.time() - start)

    print(f"Avg Torch time over {n_runs} runs: {sum(torch_times) / n_runs:.6f} s")
    print(f"Avg JAX time over {n_runs} runs: {sum(jax_times) / n_runs:.6f} s")
